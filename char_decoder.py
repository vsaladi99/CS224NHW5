...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        value, dec_hidden_return = self.charDecoder(emb, dec_hidden)
        scores = self.char_output_projection(value)

        return scores, dec_hidden_return
        ### END YOUR CODE 
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        l, bs = char_sequence.shape
        ### get the part without the end token
        scores, _ = self.forward(torch.narrow(char_sequence, 0, 0, l - 1), dec_hidden)
        scores = scores.contiguous().view((l-1)*bs, len(self.target_vocab.char2id))

        ### get the part without the start token
        target = torch.narrow(char_sequence, 0, 1, l - 1)
        target = target.contiguous().view((l - 1) * bs)

        ### get loss
        return self.loss(scores, target)
